...
|---|---|
| Themes seeded | 48 |
| Candidate proposals made (incl. gap-filling rounds) | 328 |
| Unique references sent to YouVersion for verification | 258 |
| Verified (real text fetched, checksummed) | 254 |
| Rejected at verification (reference didn't resolve) | 4 |
| Candidates fit-checked | 324 |
| Kept (passed both certainty *and* yes/no judgment) | 218 |
| Rejected at fit-check: failed the yes/no judgment | 106 |
| Rejected at fit-check: below the certainty threshold (0.35) | 0 |
| **Final verified references in `themes.json`** (deduped) | **215** |
| References per theme | min 2, max 9, avg 4.5 |

Worth flagging plainly rather than burying in the table: **every one of the 106 fit-check rejections failed on the yes/no judgment, not the certainty score.** Once the retrieval-window bug below was fixed, Gloo Search's certainty for same-domain content (184-254 short Bible verses, all thematically similar to begin with) clustered tightly in the 0.5-0.7 range regardless of actual fit — meaning the numeric embedding-similarity signal doesn't discriminate much within a corpus this narrow, and the real filtering work is being done by the independent judgment call, not the score. The certainty gate stays in the pipeline as a genuine check (an out-of-domain or truly unrelated item would still fail it), but for this specific corpus it hasn't been the deciding factor even once.

Every one of the 4 verification-stage rejections, in full:
